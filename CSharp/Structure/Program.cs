...
//언제 클래스대신 구조체를 쓰는가?
//값의 변화가 빈번하지 않게 일어날때 &&
//멤버필드 크기의 합이 16 바이트를 넘지 않을때 &&
//박싱이 거의 일어나지 않을때(박싱 : 기본타입을 object 타입으로 변환하는 과정. 언박싱 : 오브젝트 타입을 다른 자료형으로 변환하는 과정 ) &&
//기본적인 자료형들로 단일 값들을 나타낼떄 (int, float, double 등등)
public struct Stats
{
    public int STR;
    public int DEX;
...
namespace Structure
{
    internal class Program
    {
        ...
        public static void PrintCombatScore(Player player)
        {
            ...
            player.stats = new Stats(999, 999, 999, 999);    //참조해서 가져오기 때문에 값이 수정되어도 수정된 값이 출력된다.
        }

        public static void PrintCombatScore(PlayerStruct player)
        {
            Console.WriteLine(player.stats.GetCombatScore());
            player.stats = new Stats(999, 999, 999, 999);  //참조가 아닌 값을 복사해서 가져오기 때문에 수정되어도 수정된 값이 반환되지 않는다.
        }
    }
}
