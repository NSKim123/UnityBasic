...
public class PlayerStateAttack : CharacterStateBase
{
    ...
    public PlayerStateAttack(int id, GameObject owner, Func<bool> executeCondition, List<KeyValuePair<Func<bool>, int>> transition) : base(id, owner, executeCondition, transition)
    {
    ...
    public override int Update()
    {
        switch (current)
        {
            ...
            case 1:
                {
                    if (InputHandler.main.Mouse0Trigger && animator.GetBool("finishCombo") == false)
                    {
