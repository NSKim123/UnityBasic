...
public class NoteHitter : MonoBehaviour
{
    ...
    private void HitNote()
    {
        ...
        if (overlaps.Count > 0)
        {
            ...
            IOrderedEnumerable<Collider2D> colliderFiltered = overlaps.OrderBy(x => Mathf.Abs(x.transform.position.y - transform.position.y)); 
        
            //가장 가까운 콜라이더에 대해서 떨어진 거리 구하기
            float distance = Mathf.Abs(colliderFiltered.First().transform.position.y - transform.position.y);
            
            //거리에 따라서 히트 판정
            if (distance < Constants.HIT_JUDGE_RANGE_COOL/2.0f)
                hitType = HitType.Cool;
            else if (distance < Constants.HIT_JUDGE_RANGE_GREAT / 2.0f)
                hitType = HitType.Great; 
            else if (distance < Constants.HIT_JUDGE_RANGE_GOOD / 2.0f)
                hitType = HitType.Good; 
            else if (distance < Constants.HIT_JUDGE_RANGE_MISS / 2.0f)
            ...
            colliderFiltered.First().gameObject.GetComponent<Note>().Hit(hitType);
            Destroy(colliderFiltered.First().gameObject);
        }     
    }
