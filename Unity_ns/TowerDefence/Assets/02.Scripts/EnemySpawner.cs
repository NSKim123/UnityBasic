...
public class EnemySpawner : MonoBehaviour
{
    ...
    public void StartSpawn(int stage)
    {
        ...
        _stageDataList.Add(stageData);  //현재 소환중인 스테이지 데이터 리스트 등록
    ...
    private void Update()
    {
        //역순으로 for문을 진행하는 이유는 특정 스테이지 소환 종료시 리스트에서 해당 스테이지 데이터를 제거하야하기 때문
            ...
            for (int j = 0; j < _stageDataList[i].SpawnDataList.Count; j++)
            {
                ...
                if (_counterList[i][j] > 0)
                {
                    ...
                    if (_delayTimersList[i][j] <= 0)
                    {
                        ...
                        {
                            GameObject go = ObjectPool.Instance.Spawn(_stageDataList[i].SpawnDataList[j].Prefab.name
    ...
    private void RegisterPoolElement()
    {
        ...
        ObjectPool.Instance.InstantiateAllElement();
    }
    ...
    private void ActiceSkipButtons()
    {
    ...
    private void DeacticeSkipButtons()
    {
        foreach (var button in _skipButtons)
        {
