...
{
    [SerializeField] private Transform[] _filePoints;
    ...
    {
        BlinkFirePoints();
        for (int i = 0; i < _filePoints.Length; i++)
        {
            GameObject rocket = ObjectPool.Instance.Spawn("Rocket", _filePoints[i].position);
            rocket.GetComponent<ProjectileRocket>().SetUp(false, 5.0f, _damage, TargetLayer, Target);
        }
    }
    ...
    {
        foreach (Transform firePoint in _filePoints)
