...
public class Pathfinder : MonoBehaviour
{
    ...
    public bool TryFindOptimizedPath(Transform start, Transform end, out List<Transform> optimizedPath)
    {
        ...
        switch (_option)
        {
            ...
            case Options.BFS:
                {
                    found = BFS(start: TransformToCoord(start), end: TransformToCoord(end));
            ...
            case Options.DFS:
                {
                    found = DFS(start: TransformToCoord(start), end: TransformToCoord(end));
                    if (found)
    ...
    private static bool BFS(Coord start, Coord end)
    {
        ...
        Queue<Coord> queue = new Queue<Coord>(); //탐색하려는 노드들의 큐
        ...
        _path_BFS = _pathList_BFS.OrderBy(path => path.Count).First(); //list 들 중에서 가장 요소 갯수가 적은 list를 반환
        _pathList_BFS.Find(i=> i.Count == _pathList_BFS.Min(i => i.Count));
    ...
    private static List<Transform> CalcPath(List<KeyValuePair<Coord, Coord>> parents)
    {
              ...
              parents[index].Key != parents.First().Key) 
        {
            ...
            index = parents.FindLastIndex(pair => pair.Value == parents[index].Key);  //현재 부모노드를 자식노드로 가지는 조(?)부모노드의 인덱스 찾기. 못찾으면 -1을 반환함
        }
