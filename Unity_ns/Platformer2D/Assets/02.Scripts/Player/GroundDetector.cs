...
{
    public bool IsDetected => _currentGround && _currentGround != _ignoringGround;
    ...
    {
        RaycastHit2D[] hits = Physics2D.LinecastAll(transform.position, transform.position + Vector3.down * 5.0f, _groundLayer);
        if (hits.Length > 0)
        {
    ...
    public bool IgnoreCurrentGround()
    {
        ...
        _ignoreCoroutine = StartCoroutine(E_IgnoreGround(_currentGround));
        return true;    
    }
    public void StopIgnoreGround()
    {
        if(_ignoringGround != null)
        {
            StopCoroutine(_ignoreCoroutine);
