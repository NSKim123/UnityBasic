...
public class StateLadderDown : StateBase
{
    ...
    public StateLadderDown(StateMachine.StateType machineType, StateMachine machine) : base(machineType, machine)
    {
        ...
        switch (Current)
        {
            ...
            case IState.Commands.OnAction:
                {  
                    ...
                    //땅을 밟는 조건
                    else if (_rb.position.y < _ladderDetector.DownBottomStartPos.y && _groundDetector.IsDetected)
                    {
                        next = StateMachine.StateType.Idle;
                        MoveNext();
                    }
