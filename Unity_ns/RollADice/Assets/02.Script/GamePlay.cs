...
public class GamePlay : MonoBehaviour
{
    ...
    public int Direction
    {
        ...
        set
        {
            if (value <0)
            {
                ...
                _inversIcon.SetActive(true);
            }
            else
            {
                _direction = Constants.DIRECTION_POSITIVE;
                _inversIcon.SetActive(false);
            }
        }
    }
    [SerializeField] private GameObject _inversIcon;
    ...
    public void RollAGoldenDiceDice(int diceValue)
    {
        if (GoldenDiceNum > 0)
        {
            if (DiceAnimation.Instance.TryPlayDiceAnimation(diceValue, MovePlayer))
